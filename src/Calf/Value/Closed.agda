...
◯-isContr→isModal c = isoToIsEquiv (iso η• (out c) (sect c) (retr c))
  where
    ...
    sect : {X : 𝒱} (c : ◯ (isContr X)) → section η• (out c)
    sect c (η• x) = refl
    sect c (∗ abs) = law (c abs .fst) abs
    sect c (law x abs i) =
      isProp→PathP
        (λ i → isProp→isSet (●-isProp abs)
          (η• (c abs .snd x (~ i)))
    ...
    retr : {X : 𝒱} (c : ◯ (isContr X)) → retract η• (out c)
    retr c x = refl
