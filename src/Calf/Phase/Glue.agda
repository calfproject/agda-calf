...
module _ where
  ...
  glue◦-leftInv F g◦ = funExt λ p → λ i →
    ...
    where
      ...
      closed-path p = Type•-at-open-isContr (F .X•) p .snd (g◦ p .•)
      ...
      proof-path : (p : φ) →
        PathP
          (λ i → F .χ• (closed-path p i) ≡ η• (open-path p i))
...
fracture-modal : {X : Type} → ●.isModalMap (fracture {X})
fracture-modal g = ◯-isContr→isModal λ p → fracture-open-isEquiv p .equiv-proof g
...
fracture-connected : {X : Type} → isConnectedMap (fracture {X})
