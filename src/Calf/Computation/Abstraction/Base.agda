...
opaque
  ...
  triangle-Uᶜ : ∀ {A-⊤ A-abs} (α : A-⊤ ⊸ A-abs) → A-⊤ ⊸ Abstractionᶜ A-⊤ A-abs α
  ...
  triangleᶜ' : ∀ {B-⊤ B-abs} (β : B-⊤ ⊸ B-abs) (b-⊤ : U B-⊤) (b-abs : U B-abs)
    → β .U b-⊤ ≡ b-abs
    → U (Abstractionᶜ B-⊤ B-abs β)
  triangleᶜ' {B-abs = B-abs} β b-⊤ b-abs b-coherence =
...
triangle' β f-⊤ = f-⊤ ⨾ᶜ triangle-Uᶜ β

opaque
  unfolding Abstractionᶜ Abstractionᶜ-id 𝒞-glue-fracture-retract triangle-Uᶜ ⊸-path

  triangle-Uᶜ-id : PathP (λ i → A ⊸ Abstractionᶜ-id A i) (triangle-Uᶜ idᶜ) idᶜ
  triangle-Uᶜ-id {A} =
    compPathP' {B = A ⊸_}
      (⊸-path
